...
exports.get = function(id, fn){
  ...
  client.hgetall('q:job:' + job.id, function(err, hash){
    ...
    if (!hash.type) return fn(new Error("DB De-sync occured."));
...
Job.prototype.log = function(str){
  ...
  this.client.rpush('q:job:' + this.id + ':log', str, noop);
  return this;
};
 ...
 * @api public
 */
Job.prototype.progress = function(status, progress, remaining){
  if (0 == arguments.length) return this._progress;
  this.set('progress', progress);
  events.emit(this.id, 'progress', JSON.stringify({progress: progress, remaining: remaining, status:status}));
  return this;
};
...
Job.prototype.state = function(state){
  ...
  if ('inactive' == state) client.lpush('q:' + this.type + ':jobs', 1, noop);
  return this;
};
...
Job.prototype.save = function(params,fn){
  var client = this.client
    , fn = (typeof(params) == 'function') ? params : (fn || noop)
    , max = this._max_attempts
    , self = this;
  ...
  // incr id
  // add support for an externally provided ID
  var lsave = function(err, id){

    if (err) return fn(err);
    var key = 'q:job:' + id;
    self.id = id;
    self._state = self._state || 'inactive';
    if (max) client.hset(key, 'max_attempts', max, noop);

    client.sadd('q:job:types', self.type, noop);
    self.set('type', self.type);
    self.set('created_at', Date.now());
    self.update(fn);

    // add the job for event mapping
    events.add(self,function(err,subId){
      self.subId = subId;
    });
  ...
  if (params.hasOwnProperty('id')){
    lsave(null,params.id);
  }else{
    client.incr('q:ids', lsave);
  }
